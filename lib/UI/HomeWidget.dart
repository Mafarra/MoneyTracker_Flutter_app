...
import 'package:money_tracker/UI/DisplayWidget.dart';
import 'package:money_tracker/UI/SettingsWidget.dart';
import 'package:money_tracker/UI/SpendingHistory.dart';
import 'package:money_tracker/Utilitis/lists.dart';
...
class _HomeState extends State<HomeWidget> {
  ...
  void addSubscriptionEntry(SubscriptionEntry i, DateTime dt) {
    SpendingEntry subscriptionEntry = SpendingEntry();
  ...
  // This will run on startup to check if a  day has past.
  void checkNewDay() {
    // Date depends on local
    DateTime now = DateTime.now().toLocal();
    today = DateTime(now.year, now.month, now.day).millisecondsSinceEpoch;

    // If its a  day, accumulate the savings into monthly saving and reset daily
      ...
      return Scaffold(
          body: Column(
            ...
            Padding(
              ...
              child: Center(
                  child: Image(
                image: AssetImage('assets/hand_coins.jpg'),
                width: 150,
              )),
            ),
          ]));
